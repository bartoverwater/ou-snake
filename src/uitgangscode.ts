...
*/
const init = () => console.log("init called");
...
function createStartSnake() {
	const segments: number[]   = [createSegment(R + width/2, R + height/2), 
...
function createFoods() {   
   ...
   while (i < NUMFOODS ) {
     ...
     if (!food.collidesWithOneOf(snake.segment) && !food.collidesWithOneOf(foods) ) {
       foods.push(food);
       i++
     }
   }  
